...
const solution = (req, res) => {
  ...
  // console.log("The distances has", data);
  ...
  /* 5 miles */
  const maxRouteLength = 8046 + (8046 * 0.3); // meters  Note: calculation is wrong it is  // 10459.8 // 6.4 miles approx
  /* Parent Object */
  const calculations = {
  ...
  while (calculations.customersRemaining > 0) {
     ...
     */
      ...
      if (
        ...
        // Getting the current customer to where the pointer is
        const currentCustomer = calculations.currentRoute[calculations.currentRoute.length - 1].customer;

        // finding the index of currentCustomer in customers array
        currentCustomerIndex = customers.indexOf(currentCustomer);

        // finding the closest distanced customer to current customer

        // As the distances has one index ahead (has five consective sets string from store) so incementing one to compare the current customer with its relavent distances
        distances[currentCustomerIndex + 1]

        // random variables to be used
        let lowerst = 10000000000;
        let foundCustomer;
        // mapping around the relavent distances element to the customer
        distances[currentCustomerIndex + 1].elements.map((customer, i = 0) => {
          // setting the lowest value variable to the lowest value found after necessary checks
          if (lowerst >= customer.distance.value && i != currentCustomerIndex && calculations.dosfec[i].allocated != true) {
            lowerst = customer.distance.value;
            foundCustomer = customer;
            foundCustomer.index = i;
          }
          // incrementing the index to be used to find the customer name in customer array later
          i = i + 1;
          // retuing the next closet found customer after checking all objects in relavent element
          if (i === distances[currentCustomerIndex + 1].elements.length) {
            return foundCustomer;
          }
        })

        // Check if the sum of prev route distance and new customer distance exceeds the max route limit
        if (calculations.prevDistance + foundCustomer.distance.value > maxRouteLength) {
          skipForTest = true;
        }
        else {
          // puhsing into the current route object 
          const routeObject = {};
          routeObject.customer = customers[foundCustomer.index]
          routeObject.deliveryCharges = calculateCharges(foundCustomer.distance.value);
          routeObject.path = `${foundCustomer.distance.text} from ${currentCustomer} to ${routeObject.customer} `
          // console.log("The found customer name is", routeObject);
          calculations.currentRoute.push(routeObject)

          /* Set the route number to this customer */
          calculations.dosfec[foundCustomer.index].routeNumber = calculations.routeNumber;
          /* Set the total distance of the current route to be distance of this route just created  */
          calculations.prevDistance = calculations.prevDistance + foundCustomer.distance.value;
          // setting the allocated flag to true to make sure this customer is already covered in route
          calculations.dosfec[foundCustomer.index].allocated = true;

          // decremnting the customer remaing to be covered
          calculations.customersRemaining -= 1;
        ...
        calculations.deliveryRoutes.push({ totalLength: `${((calculations.prevDistance) * 0.000621371).toFixed(1)} mi from Store.`, route: calculations.currentRoute });
  ...
  log('end of solution');
  res.send({ solution: calculations.deliveryRoutes });
